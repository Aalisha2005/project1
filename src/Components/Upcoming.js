...
//     try {
//       const response = await fetch('https://api.themoviedb.org/3/movie/popular?api_key=beafc909657c51fe414a76fba559786e');
...
import '../Assets/MovieGrid.css';
import MovieDetailsModal from './MovieDetailsModal';
import MovieCard from './MovieCard'; // Assuming MovieCard is exported separately
...
const Upcoming = () => {
  ...
  const fetchMovies = async () => {
    const url = 'https://imdb-movies-web-series-etc-search.p.rapidapi.com/upcoming.json';
    const options = {
      method: 'GET',
      headers: {
    ...
    try {
      ...
      // Map the API data to your movie structure
      const movies = data.movies.map(movie => ({
        id: movie.id,
        title: movie.title,
        overview: movie.description || 'No description available.',
        vote_average: movie.rating || 'N/A',
        image: movie.poster || '', // Assuming `MovieCard` uses `movie.image`
  ...
  return (
    <div className='bodycar'>
      <div className="movie-slider-container">
        <h3 className="page-title">Upcoming Movies</h3>
        <div className="movie-slider">
          <Slider {...settings}>
            {movies.map(movie => (
              <MovieCard key={movie.id} movie={movie} onClick={handleMovieClick} />
